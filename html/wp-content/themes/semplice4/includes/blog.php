...
class semplice_blog {
	...
	public function loop($args, $page_num, $taxonomy, $is_single) {
		...
		if($query->have_posts()) {
			...
			// get pagination
			$output['html'] .= $this->pagination($query->found_posts, $page_num, $taxonomy);
	...
	// get pagination
	public function pagination($count, $page_num, $taxonomy) {
		// output
		$output = '';
		// get posts per page
		...
		//  are there more thatn 1 page?
		if($count > $posts_per_page) {
			// is first page?
			if($page_num > 1) {
				// add page num on all pages except 2
				if($page_num != 2) {
					$url = $url . '/page/' . ($page_num - 1); 
				}
				$output .= '<a href="' . $url . '">' . __('Newer Posts', 'semplice') . '</a>';
			}
			if($page_num < $pages_num) {
				$output .= '<a href="' . $url . '/page/' . ($page_num + 1) . '">' . __('Older Posts', 'semplice') . '</a>';
			}
		}
	...
	public function date_comments_link($id, $options, $permalink) {
		...
		if(get_post_type($id) == 'post') {
			...
			return '<p><a href="' . $permalink . '"' . $options['head_meta_font'] . '>' . get_the_date() . '</a> &mdash; ' . $write_comments . '</p>';
		} else {
			return '';
		}
	}
				...
				<div class="container">
					<div class="row">
						<div class="column post-column" data-xl-width="' . $options['width'] . '">
							<div class="post-heading">
								<p><a href="#"' . $options['head_meta_font'] . '>April 8, 2017</a> &mdash; <a href="#"' . $options['head_meta_font'] . '>2 Comments</a></p>
